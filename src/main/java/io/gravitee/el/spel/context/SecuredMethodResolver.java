...
/**
 * The {@link SecuredMethodResolver} is a thread-safe singleton class which can be used to replace the default {@link ReflectiveMethodResolver} used by the {@link org.springframework.expression.spel.support.StandardEvaluationContext}.
 * <p/>
...
public class SecuredMethodResolver extends ReflectiveMethodResolver {
     ...
     * Once initialized, instance of {@link SecuredMethodResolver} can be retrieved using {@link SecuredMethodResolver#INSTANCE}.
     *
    ...
    private static void loadWhitelistMethods(Environment environment) {
        ...
        if (environment != null) {
            ...
            for (Object declaration : configWhitelist) {
                parseDeclaration(String.valueOf(declaration), methods);
            }
        }
        ...
        if (loadBuiltInWhitelist) {
            ...
            try {
                while ((declaration = reader.readLine()) != null) {
                    parseDeclaration(declaration, methods);
                }
